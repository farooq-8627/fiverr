...
export default class ChatRoomServer implements Party.Server {
  ...
  /** Update the last message in the chatRooms registry */
  async updateLastMessage(message: any) {
    try {
      const lastMessage = {
        text: message.text,
      ...
      await this.party.context.parties.chatrooms.get(SINGLETON_ROOM_ID).fetch({
        method: "POST",
        body: JSON.stringify({
      ...
      console.error("📨 ChatRoom: Failed to update last message:", error);
    }
  }
  ...
  private async handleUserMessage(
    ...
    // Update the last message in the chatRooms registry
    await this.updateLastMessage(payload);
  }
